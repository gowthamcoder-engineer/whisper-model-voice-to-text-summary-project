...
def export_excel(session: Dict, path: str):
    ...
    ws.cell(row, 1).fill      = fill("F2F3F4")
...
class SpeakSenseApp(ctk.CTk):
    ...
    def _build_controls(self):
        ...
        ctk.CTkLabel(bar, text="WHISPER",
                     ...
                     text_color=MUTED).pack(side="left", padx=(18, 3))
        ...
        ctk.CTkLabel(bar, text="OLLAMA",
                     font=ctk.CTkFont("Courier", 9),
                     text_color=MUTED).pack(side="left", padx=(0, 3))
        ...
        self._btn_load = ctk.CTkButton(
            bar, text="📂 Load Audio File", width=170, height=34,
            fg_color=BLUE, hover_color="#1d4ed8",
            font=ctk.CTkFont("Segoe UI", 13, "bold"),
            command=self._load_audio_file)
        self._btn_load.pack(side="left", padx=4)
        ...
        try:
            ...
            if self._recording:
                ...
                for i, v in enumerate(self._wavedata):
                    bh = max(2, v * H * 3.5)
                    x  = i * bw
                    intensity = int(200 + v * 55)
                    blue_ch   = int(max(0, 80 - v * 80))
    ...
    def _rebuild_table(self):
        ...
        grid = self._scroll
        grid.grid_columnconfigure(0, weight=0, minsize=120)
        for i in range(self._n_speakers):
            grid.grid_columnconfigure(1 + i, weight=1)
        ...
        for i in range(self._n_speakers):
            ctk.CTkLabel(grid, text=f"SPEAKER {i+1}",
                         font=ctk.CTkFont("Courier", 10, "bold"),
                         text_color=MUTED,
                         fg_color=RAISED).grid(row=0, column=1 + i, sticky="nsew")
    ...
    def _add_segment_row(self, seg: Dict):
        grid = self._scroll
        row  = getattr(self, "_next_seg_row", 1)
        idx  = seg["speaker"]

        bucket_start = int(seg["start"] // 10) * 10
        bucket_end   = bucket_start + 10
        time_text    = f"{fmt_time(bucket_start)} → {fmt_time(bucket_end)}"
        ...
        ctk.CTkLabel(grid,
                     ...
                     text_color=MUTED,
                     fg_color=RAISED,
                     width=110).grid(row=row, column=0, padx=2, pady=2, sticky="nsew")

        for j in range(self._n_speakers):
            if j == idx:
                cell = ctk.CTkFrame(grid, fg_color=spk_bg(j), corner_radius=8,
                                    border_width=1, border_color=spk_bd(j))
                cell.grid(row=row, column=1 + j, padx=2, pady=2, sticky="nsew")
                ...
                e.grid(row=row, column=1 + j, padx=2, pady=1, sticky="nsew")
    ...
    def _build_summary_section(self):
        ...
        hdr_overall = ctk.CTkLabel(
            ...
            font=ctk.CTkFont("Segoe UI", 13, "bold"),
            text_color=GREEN, fg_color="#DCFCE7",
    ...
    def _pipeline(self, wav_path):
        try:
            ...
            self._emit("progress", (0.10, "🔄 Loading Whisper model..."))

            if self._processor is None or self._processor.model_size != wm:
                self._processor = AudioProcessor(wm)
            self._emit("progress", (0.20, "🎧 Processing audio..."))

            self._emit("progress", (0.30, "🎙 Transcribing with Whisper…"))
            data = self._processor.process(wav_path)

            self._session.update(data)
            self._emit("transcript_ready", data)

            self._emit("progress", (0.50, "📝 Generating speaker summaries..."))
            spk_summaries = {}
            ...
            self._emit("progress", (0.75, "✨ Generating overall summary…"))
            ...
            self._emit("progress", (0.90, "💾 Saving Excel…"))
    ...
    def _load_audio_file(self):
        ...
        self._status("⚙ PROCESSING FILE", AMBER)
        self._show_progress(True, 0.05, "Loading audio file…")
    ...
    def _prepare_audio(self, file_path):
        ...
        try:
            ...
            wav_path = f"temp/{uuid.uuid4().hex[:8]}.wav"
            ...
            mb.showerror("Error", f"Audio conversion failed:\n{e}")
            raise
    ...
    def _start(self):
        try:
            ...
            self._status("🔴 RECORDING", "#E53E3E")
            self._btn_start.configure(state="disabled")
            self._btn_stop.configure(state="normal", fg_color="#E53E3E")
            ...
            mb.showerror("Error", f"Recording failed:\n{e}")
    ...
    def _stop(self):
        try:
            ...
            self._btn_start.configure(state="normal", fg_color="#E53E3E")
            self._btn_stop.configure(state="disabled", fg_color="#CBD5E1")

            self._status("⚙ PROCESSING", "#D97706")
            ...
            mb.showerror("Error", f"Stop failed:\n{e}")
            raise
